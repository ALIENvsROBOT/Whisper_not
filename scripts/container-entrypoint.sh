...
WHISPER_MAX_QUEUED_REQUESTS=$(nospaces "$WHISPER_MAX_QUEUED_REQUESTS")
WHISPER_MAX_QUEUED_REQUESTS=$(noquotes "$WHISPER_MAX_QUEUED_REQUESTS")
WHISPER_QUEUE_TIMEOUT_SECONDS=$(nospaces "$WHISPER_QUEUE_TIMEOUT_SECONDS")
WHISPER_QUEUE_TIMEOUT_SECONDS=$(noquotes "$WHISPER_QUEUE_TIMEOUT_SECONDS")
...
WHISPER_CONDITION_ON_PREVIOUS_TEXT=$(nospaces "$WHISPER_CONDITION_ON_PREVIOUS_TEXT")
WHISPER_CONDITION_ON_PREVIOUS_TEXT=$(noquotes "$WHISPER_CONDITION_ON_PREVIOUS_TEXT")
WHISPER_VAD_MIN_SILENCE_MS=$(nospaces "$WHISPER_VAD_MIN_SILENCE_MS")
WHISPER_VAD_MIN_SILENCE_MS=$(noquotes "$WHISPER_VAD_MIN_SILENCE_MS")
...
WHISPER_DIARIZATION_TIMEOUT_SECONDS=$(nospaces "$WHISPER_DIARIZATION_TIMEOUT_SECONDS")
WHISPER_DIARIZATION_TIMEOUT_SECONDS=$(noquotes "$WHISPER_DIARIZATION_TIMEOUT_SECONDS")
...
[ -z "$WHISPER_MAX_QUEUED_REQUESTS" ] && WHISPER_MAX_QUEUED_REQUESTS=15
[ -z "$WHISPER_QUEUE_TIMEOUT_SECONDS" ] && WHISPER_QUEUE_TIMEOUT_SECONDS=7200
[ -z "$WHISPER_CONDITION_ON_PREVIOUS_TEXT" ] && WHISPER_CONDITION_ON_PREVIOUS_TEXT=false
[ -z "$WHISPER_VAD_MIN_SILENCE_MS" ] && WHISPER_VAD_MIN_SILENCE_MS=500
...
[ -z "$WHISPER_DIARIZATION_THREADS" ]  && WHISPER_DIARIZATION_THREADS="$WHISPER_THREADS"
[ -z "$WHISPER_DIARIZATION_TIMEOUT_SECONDS" ] && WHISPER_DIARIZATION_TIMEOUT_SECONDS=7200
...
# Accept built-in faster-whisper names, Hugging Face repository IDs, and
# container-local model paths. Reject whitespace/control characters early.
if [ -z "$WHISPER_MODEL" ] || ! printf '%s' "$WHISPER_MODEL" | grep -Eq '^[A-Za-z0-9._/@+-]+$'; then
  exiterr "WHISPER_MODEL must be a built-in model name, Hugging Face repository ID, or local path without whitespace."
fi
...
if ! printf '%s' "$WHISPER_MAX_QUEUED_REQUESTS" | grep -Eq '^(0|[1-9][0-9]*)$'; then
  exiterr "WHISPER_MAX_QUEUED_REQUESTS must be 0 or a positive integer."
fi

if ! printf '%s' "$WHISPER_QUEUE_TIMEOUT_SECONDS" | grep -Eq '^[1-9][0-9]*([.][0-9]+)?$'; then
  exiterr "WHISPER_QUEUE_TIMEOUT_SECONDS must be greater than zero."
fi

case "$WHISPER_CONDITION_ON_PREVIOUS_TEXT" in
  true|false|1|0|yes|no|on|off) ;;
  *) exiterr "WHISPER_CONDITION_ON_PREVIOUS_TEXT must be true or false." ;;
esac

if ! printf '%s' "$WHISPER_VAD_MIN_SILENCE_MS" | grep -Eq '^[1-9][0-9]*$'; then
  exiterr "WHISPER_VAD_MIN_SILENCE_MS must be a positive integer in milliseconds."
fi
...
if ! printf '%s' "$WHISPER_DIARIZATION_TIMEOUT_SECONDS" | grep -Eq '^[1-9][0-9]*([.][0-9]+)?$'; then
  exiterr "WHISPER_DIARIZATION_TIMEOUT_SECONDS must be greater than zero."
fi
...
export WHISPER_MAX_QUEUED_REQUESTS
export WHISPER_QUEUE_TIMEOUT_SECONDS
export WHISPER_LOCAL_ONLY
export WHISPER_WORD_TIMESTAMPS
export WHISPER_CONDITION_ON_PREVIOUS_TEXT
export WHISPER_VAD_MIN_SILENCE_MS
...
export WHISPER_DIARIZATION_TIMEOUT_SECONDS
...
echo
echo "Whisper_not - https://github.com/ALIENvsROBOT/Whisper_not"
...
echo "  Long ASR: condition_previous_text=$WHISPER_CONDITION_ON_PREVIOUS_TEXT vad_min_silence_ms=$WHISPER_VAD_MIN_SILENCE_MS"
echo "  Startup:  ${WHISPER_STARTUP_TIMEOUT}s timeout"
echo "  Requests: serial, ${WHISPER_MAX_QUEUED_REQUESTS} queued, ${WHISPER_QUEUE_TIMEOUT_SECONDS}s timeout"
if [ -n "$WHISPER_LOCAL_ONLY" ]; then
  echo "  Mode:     local-only (no HuggingFace downloads)"
fi
echo "  Diarize:  $WHISPER_DIARIZATION"
echo "  Diarizer: $WHISPER_DIARIZATION_DEVICE (${WHISPER_DIARIZATION_THREADS} threads)"
...
if [ -z "$WHISPER_LOCAL_ONLY" ]; then
  _model_in_cache() {
    ...
    hf_cache_name=$(printf '%s' "$m" | sed 's#/#--#g')
    [ -d "/var/lib/whisper/models--${hf_cache_name}" ] && return 0
    [ -d "$m" ] && return 0
    case "$m" in
...
echo "  https://github.com/ALIENvsROBOT/Whisper_not#production-notes"
echo
echo "Setup complete."
echo
